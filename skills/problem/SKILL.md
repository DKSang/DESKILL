...
Determine **who needs what, to make what decision** before touching any tools. This is the cheapest step to get wrong and the most expensive to skip â€” a pipeline that runs beautifully but answers no useful questions is a failed pipeline.
...
If the user doesn't know their persona, ask directly: *"Who is the end consumer â€” yourself for a portfolio project, a manager reading reports, or another system consuming the data?"*

### Step 2 â€” Write Problem Statement (3 parts)

| Part | Content |
|------|---------|
| **Context** | What are users relying on today without this pipeline? (spreadsheet, nothing, manual process?) |
| **Problem** | What question/decision is currently hard or impossible to answer, and why? List 1â€“3 specific pain points. |
| **Solution** | What will this pipeline provide â€” sources, layers, serving output â€” at a level a non-technical person can understand. |

### Step 3 â€” Define 3â€“5 Analytical Questions

Each question must:
- Be **specific enough to write SQL for today**, even without data yet.
- Have a clear entity, metric, threshold, and timeframe.
- Not be vague like "understand trends" â€” use "top 10 [entities] with [metric] change > [X]% in [Y] days".
...
### Step 4 â€” Define Success Metric

**NOT**: "Pipeline runs without errors".

**MUST BE**: "Correctly answers questions 1â€“3 with data no older than [X] [hours/days]."

Example: *"Dashboard shows correct top movers daily with data lag â‰¤ 1 business day."*

### Step 5 â€” Document Constraints
...
```markdown
# Business Problem â€” [Project Name]
...
## Solution
[1-paragraph pipeline description: sources â†’ layers â†’ serving output, in non-technical language]
...
1. [Question specific enough to write SQL â€” entity + metric + threshold/timeframe]
2. [...]
3. [...]
4. [...]  â† optional
5. [...]  â† optional
...
- [ ] 3â€“5 analytical questions specific enough to write SQL.
- [ ] Success metric is NOT "pipeline runs".
- [ ] Constraints specify budget + time.

## Next Step

After done â†’ run `/sources` to evaluate each data source and create contracts.
